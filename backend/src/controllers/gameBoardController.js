...
export const createGameBoard = async (req, res) => {
	try {
		...
		res
			.status(201)
			.json({ gameBoard: newGameBoardRes, pinCode: newGameBoardRes.pinCode });
	} catch (error) {
...
export const getGameBoard = async (req, res) => {
	try {
		const gameBoard = await GameBoard.findById(req.params.gameBoardId).populate(
			'players',
		);
		if (!gameBoard) {
...
export const loadMiniGame = async (req, res) => {
	try {
		...
		io.emit('miniGameLoaded', {
			gameBoardId: req.params.gameBoardId,
...
export const updateGameBoard = async (req, res) => {
	try {
		const gameBoard = await GameBoard.findByIdAndUpdate(
			req.params.gameBoardId,
			{ $set: req.body },
			{ new: true },
		).populate('players');
		if (!gameBoard) {
