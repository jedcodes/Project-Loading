    ...
    <div className="flex flex-col items-center justify-center p-4">
      ...
      <div className={`grid ${options.length === 3 ? 'grid-cols-2 gap-4' : 'grid-cols-2 gap-4'}`}>
        {options.map((option, index) => (
          <div 
            key={index} 
            className={`w-full max-w-md mb-2 px-10 py-2 bg-gradient-to-r from-blue-500 to-transparent text-white rounded-full flex justify-between items-center ${options.length === 3 && index === 2 ? 'col-span-2 mx-auto' : ''}`}
          >
            <span>{option}</span>
