      ...
      <div className={`grid ${currentQuestion.options.length === 3? 'grid-cols-2 gap-4' : 'grid-cols-2 gap-4'}`}>
        {currentQuestion.options.map((option: Option, index: number) =>  (
          <div 
            ...
            className={`w-full max-w-md mb-2 px-10 py-2 border-4 border-purple-500 rounded-full flex justify-between items-center cursor-pointer ${selectedOption === index ? 'bg-purple-500 text-white' : 'bg-gray-800 text-white'}`}
